...
const viPdf = path.win32.normalize('d:/SOURCES/tamling/public/images/Sản phẩm/Tiếng Việt/Trà cổ thụ đặc sản Việt Nam/Trà cổ thụ đặc sản Việt Nam.pdf');
const viOut = path.win32.normalize('d:/SOURCES/tamling/scratch/hd_vi');

const enPdf = path.win32.normalize('d:/SOURCES/tamling/public/images/Sản phẩm/English/Vietnamese specialty teas/Vietnam Specialty TEAS.pdf');
const enOut = path.win32.normalize('d:/SOURCES/tamling/scratch/hd_en');

[viOut, enOut].forEach(d => {
  if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true });
});

const viPdfB64 = Buffer.from(viPdf, 'utf8').toString('base64');
const viOutB64 = Buffer.from(viOut, 'utf8').toString('base64');
const enPdfB64 = Buffer.from(enPdf, 'utf8').toString('base64');
const enOutB64 = Buffer.from(enOut, 'utf8').toString('base64');
...
Render-PdfPagesB64 -PdfPathB64 '${viPdfB64}' -OutDirB64 '${viOutB64}' -TargetWidth 2400
Render-PdfPagesB64 -PdfPathB64 '${enPdfB64}' -OutDirB64 '${enOutB64}' -TargetWidth 2400
`;

const psFile = 'd:/SOURCES/tamling/scratch/extract_win.ps1';
fs.writeFileSync(psFile, psScript, 'utf8');

try {
  console.log("Starting WinRT PDF rendering...");
  const out = execSync(`powershell -ExecutionPolicy Bypass -File "${psFile}"`, { stdio: 'inherit' });
