const en = {
  nav: {
    home: "Home",
    about: "Our Brand",
    products: "Products",
    connect: "Connect",
  },
  hero: {
    title: "Tamling",
    storeLabel: "Online Store",
    subtitle: "Organic Teas & Herbs",
    description: "",
    cta: "Explore Products",
    learnMore: "Our Story",
  },
  about: {
    title: "About Tamling",
    subtitle: "Our Brand",
  },
  catalog: {
    title: "Product Catalog",
    subtitle: "Premium Selection",
    categories: {
      ceramic: "Vietnam's Active Trace Element & Organic Stoneware",
      incense: "Zen Incense",
      specialtyTea: "Vietnamese Specialty Teas",
      wellnessTea: "Daily Detox & Wellness Herbal Tea",
      herbalBath: "Herbal External Skin Cleansing Solution"
    },
    viewDetails: "View Brochure",
    close: "Close",
    zoom: "Zoom Image",
    prev: "Previous",
    next: "Next",
    page: "Page",
    brewingTeapot: "Traditional Teapot Brewing Guide",
    brewingPourover: "Pour-Over, Phin-Filter Brewing Guide",
    noticeTitle: "Important Notice",
    teasTitle: "8 Rare Specialty Teas",
    downloadPdf: "Download PDF Catalog"
  },
  connect: {
    title: "Connect with Tamling",
    desc: "Reach out to Tamling directly through our official channels. Contact us for orders, product enquiries, or private tasting sessions.",
    zalo: "Message via Zalo",
    messenger: "Facebook Messenger",
    whatsapp: "WhatsApp Chat",
    instagram: "Instagram"
  },
  footer: {
    copyright: "© 2026 Tamling. All rights reserved.",
  }
};

export default en;
